...
void fft_twiddle_gen(const std::launch l, int i, int i1, COMPLEX * in, COMPLEX * out, COMPLEX * W, int nW, int nWdn, int r, int m) {
	...
	auto fun = allscale::api::core::prec(
		...
		[](const params& p) { 
			fft_twiddle_gen1(p.in + p.i, p.out + p.i, p.W, p.r, p.m, p.nW, p.nWdn * p.i, p.nWdn * p.m); 
		},
		[](const params& p, const auto& rec) {
			int i2 = (p.i + p.i1) / 2;
            return parallel(
	...
	fun({i,i1,in,out,W,nW,nWdn,r,m}).get();
}
void fft_twiddle_gen_seq(int i, int i1, COMPLEX * in, COMPLEX * out, COMPLEX * W, int nW, int nWdn, int r, int m) {
	if (i == i1 - 1) {
...
void fft_aux(const std::launch l, int n, COMPLEX * in, COMPLEX * out, int *factors, COMPLEX * W, int nW)
{
	...
	if (r < n) {
		...
		*/
		if (r == 32) {
			...
			fft_unshuffle_16(l, 0, m, in, out, m);
		} else if (r == 8) {
			fft_unshuffle_8(l, 0, m, in, out, m);
		} else if (r == 4) {
			fft_unshuffle_4(l, 0, m, in, out, m);
		} else if (r == 2) {
			fft_unshuffle_2(l, 0, m, in, out, m);
		} else {
			unshuffle(l, 0, m, in, out, r, m);
		}
	...
	*/
	if (r == 2) {
		fft_twiddle_2(l, 0, m, in, out, W, nW, nW / n, m);
	} else if (r == 4) {
		fft_twiddle_4(l, 0, m, in, out, W, nW, nW / n, m);
	} else if (r == 8) {
		fft_twiddle_8(l, 0, m, in, out, W, nW, nW / n, m);
	} else if (r == 16) {
		fft_twiddle_16(l, 0, m, in, out, W, nW, nW / n, m);
	} else if (r == 32) {
		fft_twiddle_32(l, 0, m, in, out, W, nW, nW / n, m);
	} else {
		fft_twiddle_gen(l, 0, m, in, out, W, nW, nW / n, r, m);
	}
	return;
}
...
void fft(const std::launch lt, int n, COMPLEX * in, COMPLEX * out) {
	...
	inncabs::message("Computing coefficients ");
...
bool test_correctness(int n, COMPLEX *out1, COMPLEX *out2) {
	...
	std::stringstream ss;
	ss << "relative error = " << error << std::endl;
